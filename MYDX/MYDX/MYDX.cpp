// MYDX.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//
...
// DX : Direct Access -> 직접 하드웨어에 접근 (GPU)가 페인팅 하도록 하는 것
// CPU : 어려운 계산 작업 직렬처리
// GPU : 단순 계산을 병렬처리

// DX 2D
// 랜더링 파이프라인 -> 3D 공간의 어떤 좌표를 모니터에 표시하기까지 걸리는 일련의 과정

//인력 사무소장
//외주 맡기고 대표 뽑기
//컴퓨터의 하드웨어 기능 점검, 리소스 할당 하는 애들을 뽑는다.
Microsoft::WRL::ComPtr<ID3D11Device> device; //COM객체용 스마트 포인터

//연출감독
//세트장을 실질적으로 꾸며주는 연출가
//렌더링 대상 결정
//->리소스를 그래픽 파이트라인에 바인딩, 리소스 할당
ComPtr<ID3D11DeviceContext> deviceContext;

//DX의 인터페이스로써 1개 이상의 표면을 포함할 수 있다.
//각각의 표면(버퍼 텍스쳐)을 출력하기 전에 데이터를 보관한다.
//두 버퍼를 번갈아가며 그린다. 이중버퍼
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_MYDX, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_MYDX));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_MYDX));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_MYDX);
...
void InitDevice()
{
    ...
    D3D11CreateDeviceAndSwapChain(
        ...
        D3D11_SDK_VERSION,
        &sd,
        IN swapChain.GetAddressOf(), //GetAddressOf == **
    ...
    ComPtr<ID3DBlob> vertexBlob; //VertexShader 만들 때 필요한 애
