...
void Quad::CreateVertices()
{
    ...
    Vector2<float> halfSize = _srv->GetImageSize() * 0.5f;
...
void Quad::CreateData(wstring path)
{
    ...
    _vs = make_shared<VertexShader>(L"Shader/VertexShader.hlsl");
    _ps = make_shared<PixelShader>(L"Shader/PixelShader.hlsl");
