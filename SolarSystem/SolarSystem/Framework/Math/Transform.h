...
public:
	...
	void SetPosition(Vector2<float> pos) { _pos = pos; }
	void SetScale(Vector2<float> scale) { _scale = scale; }
	void SetAngle(float angle) { _angle = angle; }

	void AddVector2(Vector2<float> pos) { _pos += pos; }
	void AddScale(Vector2<float> scale) { _scale += scale; }
	void AddAngle(float angle) { _angle += angle; }

	const Vector2<float>& GetPos() { return _pos; }
	const Vector2<float>& GetScale() { return _scale; }
